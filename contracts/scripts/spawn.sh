...
export SEEDER_ADDRESS=$(cat ./manifests/dev/manifest.json | jq -r '.contracts[] | select(.name == "blobbar::blobert::seeder::Seeder" ).address')
export DESCRIPTOR_ADDRESS=$(cat ./manifests/dev/manifest.json | jq -r '.contracts[] | select(.name == "blobbar::blobert::descriptor::Descriptor" ).address')
